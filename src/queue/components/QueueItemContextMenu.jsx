...
import { playerTogglePlayPause, setTrackFromQueue } from '../../player/actions';
import { queueRemoveTrack } from '../actions';


class QueueItemContextMenu extends Component {
  handlePlayTrack = (targetNode, ref) => {
    const item = ref.props.item;
    this.props.handlePlayTrack(item.position - 1);
  };

  handleRemoveTrack = (targetNode, ref) => {
    const item = ref.props.item;
  ...
  render() {
    return (
      <ContextMenu id="queue-item-context-menu">
        <Item onClick={this.handlePlayTrack}>
          Play track
        </Item>
        <Item onClick={this.handleRemoveTrack}>
